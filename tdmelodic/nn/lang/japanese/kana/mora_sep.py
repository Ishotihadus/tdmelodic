...
def sep_katakana2mora(katakana_text=""):
    ...
    concat_ = [
        [i + j, ""]
        if i + j
        in mora_with_subs  # i + j が「キャ」「シュ」などのパターンに合致する場合
        else [i, large_vowel[j]]
        if ((i not in small_vowel) and (j in small_vowel))  # それ以外のパターンの場合。
        else ["", ""]
        if ((i in small_vowel) and (j not in small_vowel))
        else [large_vowel[i], ""]
        if ((i in small_vowel) and (j in small_vowel))
        else [i, ""]  # それ以外は普通に返す。
