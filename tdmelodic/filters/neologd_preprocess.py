...
class Preprocess(object):
    ...
    def __call__(self, fp_in, fp_out):
        ...
        NeologdPatch.message(
            "| {} Duplicate entried will{}be removed.", self.flag_rmdups
        )
        if self.flag_rmdups:
...
def my_add_argument(parser, option_name, default, help_):
    ...
    if sys.version_info >= (3, 9):
        ...
        parser.add_argument(
            "--" + option_name, action="store_true", default=default, help=help_
        )
        parser.add_argument(
            "--no-" + option_name,
            action="store_false",
...
def main():
    ...
    my_add_argument(
        parser, "rm_noisy_katakana", True, "remove noisy katakana words or not"
    )
    ...
    my_add_argument(
        parser, "rm_wrong_yomi", True, "remove words with possibly wrong yomi or not"
    )
    my_add_argument(
        parser,
        "rm_special_particle",
        True,
        'remove words with special particles "は" or "へ"',
    )
    ...
    my_add_argument(
        ...
        "normalize",
        False,
        "normalize the surface forms by applying "
    ...
    try:
        preprocess = Preprocess(
            args.rmdups, NeologdPatch(**vars(args)), dictionary_type=args.mode
        )
